...
A FastAPI REST application with role-based authentication (Tenant/Owner)
and rental recommendation logic for the RentSure web frontend.
...
# Import auth modules
from models import get_db, User, Tenant, Owner, Property, UserRole, engine, Base
from auth_routes import router as auth_router, owner_router
...
app = FastAPI(
    title="RentSure API",
    description="Production rental platform with role-based authentication",
    version="2.0.0"
)
...
def seed_demo_users():
    """Seed demo users and properties on startup"""
    from models import SessionLocal
    
    db = SessionLocal()
    try:
        # Check if demo users already exist
        tenant_demo = db.query(User).filter(User.email == "tenant@rentsure.demo").first()
        owner_demo = db.query(User).filter(User.email == "owner@rentsure.demo").first()
        ...
        if not tenant_demo:
            ...
            print("✓ Demo tenant created: tenant@rentsure.demo / Tenant@123")
        else:
            # Ensure demo tenant always has the expected demo password
            tenant_demo.password_hash = hash_password("Tenant@123")
        ...
        if not owner_demo:
            ...
            # Create demo properties for owner
            demo_properties = [
                Property(
                    owner_id=owner_user.id,
                    title="2BHK Apartment in Viman Nagar",
                    description="Spacious 2-bedroom apartment with kitchen, balcony, and parking. 24/7 security.",
                    city="pune",
                    rent=15000,
                    availability=True,
                    safety_score=4.5,
                    trust_score=4.6,
                    nearby_college="MIT-WPU",
                    college_distance_km=2.3,
                    nearby_office_hub="Hinjewadi IT Park",
                    office_distance_km=5.1,
                    women_safety_index=4.5
                ),
                Property(
                    owner_id=owner_user.id,
                    title="1 RK in Baner",
                    description="Cozy 1-room kitchen flat with attached bathroom. Good for working professionals.",
                    city="pune",
                    rent=8500,
                    availability=True,
                    safety_score=4.3,
                    trust_score=4.5,
                    nearby_college="COEP",
                    college_distance_km=4.2,
                    nearby_office_hub="Magarpatta",
                    office_distance_km=3.8,
                    women_safety_index=4.2
                ),
                Property(
                    owner_id=owner_user.id,
                    title="Studio in Kothrud",
                    description="Modern studio apartment with all amenities. Near market and public transport.",
                    city="pune",
                    rent=11000,
                    availability=True,
                    safety_score=4.6,
                    trust_score=4.7,
                    nearby_college="Symbiosis",
                    college_distance_km=1.8,
                    nearby_office_hub="Kharadi EON",
                    office_distance_km=6.2,
                    women_safety_index=4.6
                )
            ]
...
async def get_recommendations(
    ...
    """
    # Optional JWT check - if provided, verify it
    if authorization:
        from auth_utils import get_token_from_header, verify_token
        try:
            token = get_token_from_header(authorization)
            verify_token(token)
        except:
            pass  # Allow public access if no auth or invalid token
...
async def get_owner_details(owner_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    """Get owner contact details from database"""
    try:
        owner = db.query(Owner).filter(Owner.id == owner_id).first()
        if not owner:
            # Return demo owner if not found
            return {
                "id": owner_id,
                "name": "Demo Owner",
                "email": "owner@rentsure.demo",
